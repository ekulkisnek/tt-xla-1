...
def apply_rotary_emb_complex(q, k, freqs_cis):
    ...
    xq = q.astype(jnp.float32).reshape(*q.shape[:-1], -1, 2)
    xk = k.astype(jnp.float32).reshape(*k.shape[:-1], -1, 2)
    
    # Convert to complex
    xq_ = jax.lax.complex(xq[..., 0], xq[..., 1])
    xk_ = jax.lax.complex(xk[..., 0], xk[..., 1])
    
    # Apply rotation using complex multiplication
    xq_out = xq_ * freqs_cis[..., None, :]  # Broadcast over heads
    xk_out = xk_ * freqs_cis[..., None, :]
    
    # Convert back to real format
    xq_out = jnp.stack((jnp.real(xq_out), jnp.imag(xq_out)), axis=-1).reshape(*q.shape)
    xk_out = jnp.stack((jnp.real(xk_out), jnp.imag(xk_out)), axis=-1).reshape(*k.shape)
    
    return xq_out.astype(q.dtype), xk_out.astype(k.dtype)
...
def generate_text(model, params, tokenizer, max_tokens, prompt):
    ...
    for i in range(max_tokens):
        ...
        # Use model.apply with mutable cache for efficient KV cache updates
        outputs, mutable_vars = model.apply(params, input_ids=input_ids, attention_mask=attention_mask, 
                             position_ids=position_ids, past_key_values=past_key_values, return_dict=True, mutable=["cache"])
...
def main():
    ...
    output, peak_mem, avg_time_per_token = generate_text(model, params, tokenizer, 50, dog_food_prompt)
