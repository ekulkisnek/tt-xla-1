...
# --- Generation ---
def sample_next_token(logits, generated_tokens=None, top_p=0.9):
    """Sample with nucleus sampling (top-p) to reduce repetition."""
    # Apply repetition penalty to recently generated tokens
    if generated_tokens and len(generated_tokens) > 0:
        for token_id in generated_tokens[-3:]:  # Penalize last 3 tokens
            if token_id < logits.shape[-1]:
                logits = logits.at[..., token_id].set(logits[..., token_id] * 0.8)
    ...
    indices_to_remove = jnp.take_along_axis(sorted_indices_to_remove, sorted_indices, axis=-1)
