...
def sample_next_token(logits, generated_tokens=None):
    """Aggressive anti-repetition sampling for tensor parallelism."""
    # Apply aggressive anti-repetition penalty
    if generated_tokens and len(generated_tokens) > 0:
        # Get the last few tokens to avoid repetition
        recent_tokens = generated_tokens[-5:]  # Last 5 tokens
        for token_id in recent_tokens:
            if token_id < logits.shape[-1]:
                # More aggressive penalty - set to very negative value
                logits = logits.at[..., token_id].set(-1e9)
    
    # Use greedy sampling with aggressive anti-repetition penalty
