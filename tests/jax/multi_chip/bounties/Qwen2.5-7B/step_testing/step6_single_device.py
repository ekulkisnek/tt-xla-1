...
class ParallelDense(nn.Module):
    """Regular dense layer for single device execution."""
    features: int
    ...
    def __call__(self, x):
        ...
        # Simple matrix multiplication for single device
        output = jnp.einsum("bsd,df->bsf", x, kernel)
...
# --- Generation ---
def sample_next_token(logits, generated_tokens=None, repetition_penalty=1.1):
    """Sample with repetition penalty to reduce repetitive outputs."""
    if generated_tokens and len(generated_tokens) > 0:
        # Apply repetition penalty to recently generated tokens
        for token_id in generated_tokens[-5:]:  # Penalize last 5 tokens
            if token_id < logits.shape[-1]:
                logits = logits.at[..., token_id].set(logits[..., token_id] / repetition_penalty)
    
    # Use greedy sampling but with repetition penalty
...
def generate_text(model, params, tokenizer, max_tokens, prompt):
    ...
    # Use a different prompt format to avoid repetition
    formatted_text = f"<|im_start|>system\nYou are a helpful math tutor. Solve the problem step by step.<|im_end|>\n<|im_start|>user\n{prompt}<|im_end|>\n<|im_start|>assistant\n"
