...
# --- Generation ---
def sample_next_token(logits, generated_tokens=None, step=0):
    """Tensor parallelism compatible sampling with anti-repetition."""
    # Apply anti-repetition penalty to recently generated tokens
    if generated_tokens and len(generated_tokens) > 0:
        # Get the last few tokens to avoid repetition
        recent_tokens = generated_tokens[-3:]  # Last 3 tokens
        for token_id in recent_tokens:
            if token_id < logits.shape[-1]:
                # Reduce probability of repeating recent tokens
                logits = logits.at[..., token_id].set(logits[..., token_id] * 0.5)
    
    # Use step-dependent sampling to break repetition
...
def generate_text(model, params, tokenizer, max_tokens, prompt):
    ...
    for i in range(max_tokens):
        ...
        next_token = sample_next_token(logits[:, -1, :], generated_tokens, i)
...
def main():
    ...
    sam_question = "Solve this math problem: Sam got 80 and 90 on two tests. For an average of 85, what must the third test score be?"
