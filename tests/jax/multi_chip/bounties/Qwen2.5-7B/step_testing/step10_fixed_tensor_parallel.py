...
class ParallelDense(nn.Module):
    ...
    def __call__(self, x):
        ...
        def matmul_fn(x, k, b=None):
            ...
            full_out = jax.lax.all_gather(local_out, axis_name="mp", axis=0, tiled=True)
...
# --- Generation ---
def sample_next_token(logits, generated_tokens=None):
    """Top-2 sampling for tensor parallelism compatibility."""
    # Get top-2 tokens
    top_2_values, top_2_indices = jax.lax.top_k(logits, 2)
    
    # If we have recent tokens, avoid the most recent one
    if generated_tokens and len(generated_tokens) > 0:
        last_token = generated_tokens[-1]
        # If the top token is the same as the last token, use the second best
        if top_2_indices[0, 0] == last_token:
            return int(top_2_indices[0, 1])
    
    # Otherwise use the top token
    return int(top_2_indices[0, 0])
...
def generate_text(model, params, tokenizer, max_tokens, prompt):
    ...
    for i in range(max_tokens):
        ...
        next_token = sample_next_token(logits[:, -1, :], generated_tokens)
...
def main():
    ...
    sam_question = "Calculate: Sam has test scores of 80 and 90. What third score gives an average of 85?"
