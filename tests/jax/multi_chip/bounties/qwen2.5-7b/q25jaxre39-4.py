...
def comprehensive_equivalence_test(jax_model, jax_params, jax_tokenizer, pytorch_model, pytorch_tokenizer,
    ...
    is_gpu = check_gpu()
    jax_timeout = 60 if is_gpu else 120  # Extended for CPU
    layer_timeout = 300 if is_gpu else 600  # Extended for CPU
    if not is_gpu:
        print("Extended timeouts for CPU")
    ...
    if args.compare_sampling:
        try:
            ...
            if args.expanded_tests:
                ...
                # Equivalent if core tests pass and (layer passes OR layer was skipped)
