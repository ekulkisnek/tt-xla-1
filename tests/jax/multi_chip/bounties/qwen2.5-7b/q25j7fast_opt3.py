...
- Speedups: JIT on model.apply, pre-tokenize prompts, optional verbose logging, XLA flags, disable debug NaNs, GC every 10 tokens.

Usage:
python q25j7fast_opt3.py --model_path weights [--verbose]
"""
import os
import sys
import json
...
# XLA flags for CPU optimization
...
def precompute_freqs_cis(dim: int, end: int, theta: float = 500000.0):
    ...
    Returns:
        cos and sin values of shape [end, dim//2]
    """
...
def precompute_causal_mask(max_seq_len: int):
    ...
    Returns:
        Causal mask of shape [1, 1, max_seq_len, max_seq_len]
    """
    # Create causal mask for maximum sequence length
    causal_mask = make_causal_mask(max_seq_len, max_seq_len)
    # Add batch and head dimensions: [1, 1, max_seq_len, max_seq_len]
...
class QwenAttention(nn.Module):
    ...
    def setup(self):
        ...
        self.cos_cache, self.sin_cache = precompute_freqs_cis(
            self.head_dim,  # dim
            c.get("max_sequence_length", 32768) * 2,  # end (safety factor)
    ...
    def __call__(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None):
        ...
        # OPTIMIZATION 1: Apply rotary embeddings using precomputed values
        if position_ids is not None:
            # Just lookup precomputed values
            cos_lookup = jnp.take(self.cos_cache, position_ids[0], axis=0)  # [seq_len, dim//2]
            sin_lookup = jnp.take(self.sin_cache, position_ids[0], axis=0)  # [seq_len, dim//2]
        ...
        # Use float32 for attention scores to save memory
...
# Keep the old functions for backward compatibility (not used in optimized version)
...
def apply_rotary_emb_old(q, k, cos, sin):
...
class Qwen25ForCausalLM(nn.Module):
    ...
    def __call__(self, input_ids, attention_mask=None, position_ids=None, past_key_values=None, return_dict=True):
        ...
        if key_len <= self.causal_mask.shape[-1]:
            # Use precomputed mask and slice it
            ...
            # Fallback to computing mask on-the-fly for very long sequences
...
# --- Main ---
def main():
    parser = argparse.ArgumentParser(description="Qwen2.5-7B-Instruct JAX Inference for GSM8K Benchmark - OPTIMIZATION 3")
    ...
    # OPTIMIZATION 2: JIT compile model.apply for speed
    ...
    print("\nPre-tokenizing all GSM8K samples...")
    gsm8k_samples = [
        ...
        {"question": "Alexis is applying for a new job and bought a new set of business clothes to wear to the interview. She went to a department store with a budget of $200 and spent $30 on a button-up shirt, $46 on suit pants, $38 on suit coat, $11 on socks, and $18 on a belt. She also purchased a pair of shoes, but lost the receipt for them. She has $16 left from her budget. How much did Alexis pay for the shoes?", "answer": "41"},
    ...
    for sample in gsm8k_samples:
        ...
        pre_tokenized.append((input_ids, position_ids, sample))
    ...
    for i, (input_ids, position_ids, sample) in enumerate(pre_tokenized):
        print(f"\n--- Sample {i+1}/{len(pre_tokenized)}: {sample['question']} ---")
        gc.collect()  # GC before each sample
        output, peak_mem, avg_time_per_token = generate_text(model, jit_apply, params, tokenizer, 256, input_ids, position_ids, args.verbose)
    ...
    avg_peak_mem = total_peak_mem / len(pre_tokenized)
    avg_time_per_token = total_avg_time_per_token / len(pre_tokenized)
    accuracy = correct_count / len(pre_tokenized) * 100
    print("\nBenchmark Summary:")
    print(f"Accuracy: {accuracy:.2f}% ({correct_count}/{len(pre_tokenized)} correct)")
