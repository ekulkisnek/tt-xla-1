#!/usr/bin/env python3
"""
Optimization 7: Efficient Attention Bias with lax.select
This implements lax.select for more efficient conversion of boolean masks to attention bias values.
"""
import os
import sys
import json
...
logger = logging.getLogger("qwen25_opt7")
...
class QwenAttention(nn.Module):
    ...
    def __call__(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None):
        ...
        if attention_mask is not None:
            # Transform boolean mask into float mask using lax.select
            attention_bias = lax.select(
                attention_mask > 0,  # Condition: where mask is True
                jnp.full(attention_mask.shape, 0.0).astype(self.dtype),  # True case: 0.0
                jnp.full(attention_mask.shape, jnp.finfo(self.dtype).min).astype(self.dtype),  # False case: -inf
            )
...
def main():
    parser = argparse.ArgumentParser(description="Qwen2.5-7B JAX Inference - Optimization 7: Efficient Attention Bias with lax.select")
