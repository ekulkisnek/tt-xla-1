#!/usr/bin/env python3
"""
Optimization 8: Advanced Attention with jax.nn.dot_product_attention
This implements the optimized attention using JAX's built-in dot_product_attention function.
"""
import os
import sys
import json
...
logger = logging.getLogger("qwen25_opt8")
...
class QwenAttention(nn.Module):
    ...
    def __call__(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None):
        ...
        # GQA: Repeat k/v to match query heads (before transpose)
        if self.num_heads != self.num_kv_heads:
            repeat = self.num_heads // self.num_kv_heads
            k = jnp.repeat(k, repeat, axis=2)  # repeat heads
            v = jnp.repeat(v, repeat, axis=2)
        # Now k, v: [batch, seq, num_heads, head_dim]

        # Transpose all to [batch, num_heads, seq, head_dim]
        q = q.transpose(0, 2, 1, 3)
        k = k.transpose(0, 2, 1, 3)
        v = v.transpose(0, 2, 1, 3)

        # Convert to float32 for dot_product_attention (it expects float32)
        q_f32 = q.astype(jnp.float32)
        k_f32 = k.astype(jnp.float32)
        v_f32 = v.astype(jnp.float32)

        # Use optimized dot_product_attention without bias for now
...
def main():
    parser = argparse.ArgumentParser(description="Qwen2.5-7B JAX Inference - Optimization 8: Advanced Attention with dot_product_attention")
