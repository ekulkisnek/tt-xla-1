#!/usr/bin/env python3
"""
Step 2: Test full model with exact Llama attention and real weights
"""
import os
import jax
...
class ExactLlamaAttention(nn.Module):
    """EXACT copy of Llama's attention with working rotary embeddings"""
    ...
    def setup(self):
        ...
        # Use ParallelDense for all projections (Llama logic, Qwen naming)
    ...
    def __call__(
        ...
        # EXACT copy of Llama's attention computation with Qwen parameter names
...
class LlamaStyleDecoderLayer(nn.Module):
    """Decoder layer using Llama-style attention"""
    config: Dict[str, Any]
    dtype: jnp.dtype = jnp.float32
    ...
    def setup(self):
        ...
        self.self_attn = ExactLlamaAttention(config=llama_config, dtype=jnp.bfloat16, param_dtype=jnp.bfloat16)
...
class LlamaStyleQwenModel(nn.Module):
    """Full Qwen model with Llama-style attention"""
    config: Dict[str, Any]
    dtype: jnp.dtype = jnp.float32
    ...
    def setup(self):
        ...
        self.layers = [LlamaStyleDecoderLayer(config=c, dtype=jnp.bfloat16, name=f"layers_{i}") for i in range(c["num_hidden_layers"])]
...
def test_llama_style_full_model():
    """Test Step 2: Full model with Llama-style attention and real weights"""
    ...
    print("=== STEP 2: TESTING LLAMA-STYLE FULL MODEL ===\n")
    print("Goal: Test if Llama attention flow produces correct results with real weights")
    
    # Create model
    model = LlamaStyleQwenModel(config=config, dtype=jnp.bfloat16)
    ...
    try:
        ...
        if next_token == 220:  # Token 220 is ' ' (space)
            print("🎉 STEP 2 SUCCESS: Llama-style attention produces CORRECT output!")
            print("This means the exact Llama attention flow is compatible with Qwen!")
            return True
        else:
            print(f"❌ Step 2 partial success: Model works but output differs")
            print(f"Expected: 220 (' '), Got: {next_token} ('{next_token_text}')")
            print("This means there might be subtle differences to investigate")
            return "partial"
...
if __name__ == "__main__":
    result = test_llama_style_full_model()
    
    if result == True:
        print("\n✅ STEP 2 COMPLETE: Ready for Step 3 (replace current attention)")
    elif result == "partial":
        print("\n⚠️ STEP 2 PARTIAL: Need to investigate remaining differences")
    else:
        print("\n❌ STEP 2 FAILED: Need to debug Llama-style model integration")