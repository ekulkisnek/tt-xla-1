...
def compare_first_token_sampling(jax_model, jax_params, jax_tokenizer, pytorch_model, pytorch_tokenizer, 
    ...
    # Sample from JAX logits
    jax_next_token = sample_next_token(jax_last_logits, temperature, top_p, top_k, repetition_penalty, jax_input_ids[0].tolist(), seed=0)
    ...
    if jax_token_id != pytorch_token_id:
        print(f"\n--- LOGITS DIFFERENCES ---")
        print(f"JAX logits range: [{jax_last_logits.min():.6f}, {jax_last_logits.max():.6f}]")
        print(f"PyTorch logits range: [{pytorch_last_logits.min():.6f}, {pytorch_last_logits.max():.6f}]")
        ...
        for i, idx in enumerate(jax_top_k_indices[0]):
            ...
            print(f"  {i+1}. ID {int(idx)} ('{token_text}'): {jax_last_logits[0, int(idx)]:.6f}")
        ...
        for i, idx in enumerate(pytorch_top_k_indices[0]):
            ...
            print(f"  {i+1}. ID {int(idx)} ('{token_text}'): {pytorch_last_logits[0, int(idx)]:.6f}")
