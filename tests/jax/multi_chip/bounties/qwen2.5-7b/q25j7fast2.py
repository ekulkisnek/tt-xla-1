...
- Speedups: JIT on model.__call__, pre-tokenize prompts, optional verbose logging (every 10 tokens), XLA flags, disable debug NaNs, GC every 10 tokens, lax.scan for gen loop, KV cache prefill, cap max_tokens=200.

Usage:
python q25j7fast.py --model_path weights [--verbose]
"""
import os
import sys
import json
...
def main():
    ...
    for i, (input_ids, position_ids, sample) in enumerate(pre_tokenized):
        ...
        output, peak_mem, avg_time_per_token = generate_text(model, jit_apply, params, tokenizer, 200, input_ids, position_ids, args.verbose)
        ...
        # Progress estimate
        elapsed = time.perf_counter() - start_benchmark_time
        remaining_samples = len(pre_tokenized) - (i + 1)
        est_remaining = (elapsed / (i + 1)) * remaining_samples if (i + 1) > 0 else 0
