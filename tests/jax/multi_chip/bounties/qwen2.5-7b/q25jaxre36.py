#!/usr/bin/env python3
"""
Self-contained Qwen2.5-7B-Instruct inference script for single-device JAX with comprehensive testing.
...
- Comprehensive PyTorch equivalence testing with expanded validation.
- Matches PyTorch script behavior with guaranteed alignment.

Usage:
python q25jaxre36.py --model_path weights --prompt "Janet's dogs eat 2 pounds of dog food each day. If Janet buys a 50-pound bag of dog food, how many days will it last?" --max_tokens 256 --temperature 0.7 --top_p 0.9 --top_k 50 --dtype bfloat16 --compare_sampling --expanded_tests
"""
import os
import sys
import time
...
def comprehensive_equivalence_test(jax_model, jax_params, jax_tokenizer, pytorch_model, pytorch_tokenizer,
                                 test_prompts=None, num_seeds=3, max_steps=5, temperature=0.7, top_p=0.9, 
                                 top_k=50, repetition_penalty=1.1, test_dtype=None):
    """Comprehensive equivalence testing between JAX and PyTorch implementations"""
    ...
    for prompt_idx, test_prompt in enumerate(test_prompts):
        ...
        if jax_logits_np.shape != pytorch_logits_np.shape:
            ...
            if logits_close:
                ...
                passed_tests += 1
                test_results.append(("logits_close", True, "Logits match"))
            else:
                # Compute statistics
                logits_diff = np.abs(jax_logits_np - pytorch_logits_np)
                mean_diff = np.mean(logits_diff)
                max_diff = np.max(logits_diff)
                print(f"    ❌ Logits differ: mean_diff={mean_diff:.6f}, max_diff={max_diff:.6f}")
                test_results.append(("logits_close", False, f"mean_diff={mean_diff:.6f}, max_diff={max_diff:.6f}"))
        ...
        for seed in range(num_seeds):
            ...
            if jax_token_id == pytorch_token_id:
                print(f"    ✅ Seed {seed}: Token match (ID {jax_token_id})")
                passed_tests += 1
                test_results.append((f"token_seed_{seed}", True, f"Token ID {jax_token_id}"))
            else:
                ...
                print(f"    ❌ Seed {seed}: Token mismatch - JAX: {jax_token_id} ('{jax_text}') vs PyTorch: {pytorch_token_id} ('{pytorch_text}')")
                test_results.append((f"token_seed_{seed}", False, f"JAX:{jax_token_id} vs PT:{pytorch_token_id}"))
        ...
        try:
            ...
            if jax_generated == pytorch_generated:
                print(f"    ✅ Autoregressive sequences match")
                passed_tests += 1
                test_results.append(("autoregressive", True, f"Generated {len(jax_generated)} tokens"))
            else:
                print(f"    ❌ Autoregressive sequences differ")
                print(f"      JAX: {jax_generated}")
                print(f"      PyTorch: {pytorch_generated}")
                test_results.append(("autoregressive", False, "Sequence mismatch"))
        ...
        try:
            ...
            if args.expanded_tests:
                ...
                all_passed, test_results = comprehensive_equivalence_test(
                    ...
                    num_seeds=3, max_steps=5,
                    temperature=args.temperature, top_p=args.top_p, top_k=args.top_k,
                    repetition_penalty=args.repetition_penalty, test_dtype=args.dtype
                )
