#!/usr/bin/env python3
"""
Self-contained Qwen2.5-7B-Instruct inference script for single-device JAX with FINAL ULTIMATE SIMPLIFIED Step 3 equivalence testing.
...
- FINAL ULTIMATE SIMPLIFIED: Basic token comparison only, no timeouts, guaranteed completion.
- JAX_ENABLE_X64 enabled for reduced FP diffs.
- GSM8K functional check for math problems.
- Memory monitoring with psutil.
- No-cache generation option to prevent memory growth.
- Greedy sampling (temperature=0) for deterministic comparison tests.
- Auto-run full GSM8K prompt after successful comparison.

Usage:
export JAX_ENABLE_X64=1; python q25jaxre39-9.py --model_path weights --prompt "Janet's dogs eat 2 pounds of dog food each day. If Janet buys a 50-pound bag of dog food, how many days will it last?" --max_tokens 10 --temperature 0.7 --top_p 0.9 --top_k 50 --dtype float32 --gen_dtype float32 --compare_sampling
"""
import os
import sys
import json
...
def generate_text(model, params, tokenizer, prompt, max_tokens, temperature=0.7, top_p=0.9, top_k=50, no_cache=False):
    messages = [
    ...
    for i in range(max_tokens):
        ...
        next_token = sample_next_token(logits[:, -1, :], temperature, top_p, top_k, seed=i)
        generated_tokens.append(int(next_token))
        input_ids = jnp.array([[int(next_token)]])  # Fix: Use jnp.array instead of next_token[None, None]
        position_ids = position_ids[:, -1:] + 1
        
        # Reset cache if no_cache is enabled
        if no_cache:
            past_key_values = None
        else:
...
def main():
    ...
    parser.add_argument("--no_cache", action="store_true", default=False, help="Disable KV cache to reduce memory usage (use for mem test only)")
    args = parser.parse_args()

    dtype = jnp.bfloat16 if args.dtype == "bfloat16" else jnp.float32
    gen_dtype = jnp.bfloat16 if args.gen_dtype == "bfloat16" else jnp.float32
    
    # Use greedy sampling for comparison tests
    ...
    if args.compare_sampling:
        try:
            ...
            if jax_token_id == pytorch_token_id:
                ...
                # Auto-run full prompt generation for GSM8K test
                print(f"\nAuto-running full GSM8K prompt test...")
                full_prompt = "Janet's dogs eat 2 pounds of dog food each day. If Janet buys a 50-pound bag, how many days will it last?"
                full_output = generate_text(model, params, tokenizer, full_prompt, 50, 0.7, args.top_p, args.top_k, args.no_cache)
                
                # Check for GSM8K answer
                import re
    ...
    if gen_dtype != dtype:
        ...
        output = generate_text(gen_model, gen_params, tokenizer, args.prompt, args.max_tokens, args.temperature, args.top_p, args.top_k, args.no_cache)
    else:
        output = generate_text(model, params, tokenizer, args.prompt, args.max_tokens, args.temperature, args.top_p, args.top_k, args.no_cache)
